...
public class UsbSocketReceiver : MonoBehaviour
{
    ...
    private void AcceptLoop()
    {
        while (_running)
        {
            ...
            try
            {
                ...
                byte[] buf = new byte[RECEIVERBUFFER];
    ...
    private void ParseBlock(string block, (float x, float z)[] cachedPocketsXZ)
    {
        ...
        while (start < span.Length)
        {
            ...
            if (!line.IsEmpty)
            {
                ...
                {
                    string environmentJsonData = body.ToString().Trim().Replace(".json", string.Empty);
                    ...
                    string resourcePath = $"TableConfigurations/{environmentJsonData}";
                    ...
                    else
                    {
                        ...
                        else
                        {
                            try
                            {
                                EnvironmentInfo newEnvironment = JsonConvert.DeserializeObject<EnvironmentInfo>(data);
                ...
                else if (token.SequenceEqual("p"))
                {
                    if (TryParsePockets(body, out (float x, float z)[] parsedPockets))
                    {
                        ...
                        {
                            parsedPocketSnapshot.Add(
                                new DetectedPocketSnapshot2D(
                                    i,
                                    new Vector2Float(parsedPockets[i].x, parsedPockets[i].z)));
                        }

                        if (VerboseLogs)
                            Debug.Log("[USB] Parsed raw 6-pocket snapshot.");
                    }
                    else if (VerboseLogs)
                    {
                        Debug.LogWarning($"[USB] Ignored malformed pocket line: '{line.ToString()}'");
                    }
                }
                else if (token.SequenceEqual("s"))
                {
                    if (TryParseCueStick(body, out DetectedCueStickSnapshot2D cueStickSnapshot))
                    {
                        parsedCueStickSnapshot = cueStickSnapshot;
                        hasParsedCueStickSnapshot = true;

                        if (VerboseLogs)
                        {
                            Debug.Log(
                                "[USB] Parsed cue stick: " +
                                $"line=({cueStickSnapshot.LinePointXZ.X:F3},{cueStickSnapshot.LinePointXZ.Y:F3}) " +
                                $"dir=({cueStickSnapshot.DirectionXZ.X:F4},{cueStickSnapshot.DirectionXZ.Y:F4}) " +
                                $"hit=({cueStickSnapshot.HitPointXZ.X:F3},{cueStickSnapshot.HitPointXZ.Y:F3}) " +
                                $"conf={cueStickSnapshot.Confidence:F2}");
                        }
                    }
                    else if (VerboseLogs)
                    {
        ...
        if (!hasRawSnapshotContent)
            return;

        svc.ApplyPythonRawSnapshot(
            _lastAppliedEnvironmentKey,
            parsedPocketSnapshot,
    ...
    {
        parsedPockets = null;
        const byte expectedPocketCount = 6;

        (float x, float z)[] provisionalPockets = new (float x, float z)[expectedPocketCount];
        byte parsedPocketCount = 0;
        ReadOnlySpan<char> pocketBody = body.Trim();

        while (parsedPocketCount < expectedPocketCount && !pocketBody.IsEmpty)
        {
            TryNextToken(ref pocketBody, ';', out ReadOnlySpan<char> pair);
            if (pair.IsEmpty)
                break;

            TryNextToken(ref pair, ',', out ReadOnlySpan<char> xS);
            ReadOnlySpan<char> zS = pair;
            ...
            provisionalPockets[parsedPocketCount] = (x, z);
            parsedPocketCount++;
        }

        if (parsedPocketCount != expectedPocketCount)
            return false;
        ...
        bool isLoadedFromBackup = false)
    {
        ...
        {
            Debug.Log(
                $"[USB] Table(t): L={playfieldLength:F3}m,\r\n" +
    ...
    {
        cueStickSnapshot = default;

        ReadOnlySpan<char> payload = body.Trim();
        if (payload.IsEmpty)
            return false;

        if (!TryNextToken(ref payload, ';', out ReadOnlySpan<char> linePointRaw))
            return false;

        if (!TryNextToken(ref payload, ';', out ReadOnlySpan<char> directionRaw))
            return false;

        if (!TryNextToken(ref payload, ';', out ReadOnlySpan<char> hitPointRaw))
            return false;

        ReadOnlySpan<char> confidenceRaw = payload.Trim();

        if (!TryParseVector2Float(linePointRaw, out Vector2Float linePointXZ))
            return false;

        if (!TryParseVector2Float(directionRaw, out Vector2Float directionXZ))
            return false;

        if (!TryParseVector2Float(hitPointRaw, out Vector2Float hitPointXZ))
            return false;

        if (!TryParseFloat(confidenceRaw, out float confidence))
            return false;

        cueStickSnapshot = new DetectedCueStickSnapshot2D(
            linePointXZ,
            directionXZ,
            hitPointXZ,
            confidence);

        return cueStickSnapshot.IsValid();
    }
    ...
    private static bool TryParseVector2Float(ReadOnlySpan<char> raw, out Vector2Float value)
    {
        ...
        if (!TryNextToken(ref pair, ',', out ReadOnlySpan<char> xS))
            return false;

        ReadOnlySpan<char> yS = pair;
    ...
    private static float MmToM(float valueMm) => valueMm > 0f ? valueMm / 1000f : valueMm;
    ...
    {
        (float x, float z)[] localPocketsXZ = null;
        EnsureSvc();
